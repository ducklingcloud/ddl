...
var FCKEnterKey = function(targetWindow, enterMode, shiftEnterMode, tabSpaces) {
	...
	oKeystrokeHandler.SetKeystrokes([ [ 13, 'Enter' ], [ SHIFT + 13, 'ShiftEnter' ], [ 8, 'Backspace' ],
			[ CTRL + 8, 'CtrlBackspace' ], [ 46, 'Delete' ] ]);
...
FCKEnterKey.prototype.DoBackspace = function() {
	...
	if (oRange.StartBlockLimit == oRange.EndBlockLimit && oStartBlock && oEndBlock) {
		...
		if (oRange.CheckStartOfBlock()) {
			...
			var ePrevious = FCKDomTools.GetPreviousSourceElement(oCurrentBlock, true, [ 'BODY',
					oRange.StartBlockLimit.nodeName ], [ 'UL', 'OL' ]);
...
FCKEnterKey.prototype.DoDelete = function() {
	...
	if (oRange.CheckIsCollapsed() && oRange.CheckEndOfBlock(FCKBrowserInfo.IsGeckoLike)) {
		...
		var eNext = FCKDomTools.GetNextSourceElement(oCurrentBlock, true, [ oRange.StartBlockLimit.nodeName ], [ 'UL',
				'OL', 'TR' ], true);
	...
	if (oSplitInfo) {
		...
		if (!bIsStartOfBlock && !bIsEndOfBlock) {
			...
			if (eNextBlock.nodeName.IEquals('li') && eNextBlock.firstChild
				...
				eNextBlock.insertBefore(FCKTools.GetElementDocument(eNextBlock).createTextNode('\xa0'),
						eNextBlock.firstChild);
		...
		if (FCKBrowserInfo.IsGeckoLike) {
			if (eNextBlock) {
				...
				span.innerHTML = '&nbsp;';
				oRange.InsertNode(span);
				FCKDomTools.ScrollIntoView(span, false);
				oRange.DeleteContents();
			} else {
...
FCKEnterKey.prototype._CheckIsAllContentsIncluded = function(range, node) {
	...
	if (range.EndContainer == node || range.EndContainer == node.lastChild) {
		var nodeLength = range.EndContainer.nodeType == 3 ? range.EndContainer.length

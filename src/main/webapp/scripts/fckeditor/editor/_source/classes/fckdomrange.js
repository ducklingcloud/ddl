...
FCKDomRange.prototype = {
		...
		if (this.StartBlockLimit == this.EndBlockLimit) {
			...
			if (eStartBlock && eEndBlock && eStartBlock == eEndBlock) {
				if (bIsEndOfBlock) {
					...
					if (FCKBrowserInfo.IsGecko && !eStartBlock.nodeName.IEquals([ 'ul', 'ol' ]))
	...
	MoveToSelection : function() {
		//使用API存在与否来判断，IE10中Selection的API已经是W3C标准的了
	...
	Select : function(forceExpand) {
		if (FCKBrowserInfo.IsGecko || FCKBrowserInfo.IsGeckoLike) {
			...
			if (oRange) {
				...
				var newRange = this.Window.document.createRange();
				newRange.setStart(startContainer, oRange.startOffset);
				try {
					newRange.setEnd(oRange.endContainer, oRange.endOffset);
				} catch (e) {
					if (e.toString().Contains('NS_ERROR_ILLEGAL_VALUE')) {
						oRange.collapse(true);
						newRange.setEnd(A.endContainer, oRange.endOffset);
					} else
						throw (e);
				}
				var seletion = this.Window.getSelection();
				seletion.removeAllRanges();
				seletion.addRange(newRange);
			}
	...
	SelectBookmark : function(bookmark, forceExpand) {
		if (FCKBrowserInfo.IsGecko ||FCKBrowserInfo.IsGeckoLike) {
			...
			if (eEndMarker) {
				...
				// Move the end boundary of the main range to match the tool
				// range.
				...
				// Append a temporary <span>&#65279;</span> before the
				// selection.
				// This is needed to avoid IE destroying selections inside empty
				// inline elements, like <b></b> (#253).
				// It is also needed when placing the selection right after an
				// inline
				...
				if (bIsStartMakerAlone) {
					...
					// instead to have any char, which will be later deleted
					// using the
					// selection.
			...
			// Remove the markers (reset the position, because of the changes in
			// the DOM tree).
	...
	_GetSelectionMarkerTag : function(toStart) {
		var doc = this.Window.document;
		...
		// IE might take the range object to the main window instead of inside
		// the editor iframe window.
		...
		// Paste a marker element at the collapsed range and get it from the
		// DOM.

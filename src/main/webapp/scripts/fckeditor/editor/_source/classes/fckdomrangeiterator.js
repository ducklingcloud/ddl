	...
	GetNextParagraph : function() {
		...
		while (currentNode) {
			...
			if (!includeNode) {
				...
				if (boundarySet[nodeName] && (!FCKBrowserInfo.IsIE7 || currentNode.scopeName == 'HTML')) {
			...
			if ((closeRange || isLast) && range) {
				...
				if (range.StartNode == range.EndNode && range.StartNode.parentNode == range.StartBlockLimit
		...
		if (!block) {
			...
			if (!block && !this.EnforceRealBlocks && range.StartBlockLimit.nodeName.IEquals('DIV', 'TH', 'TD')
				...
				this._NextNode = block == lastNode ? null : FCKDomTools.GetNextSourceNode(range.EndNode, true, null,
						lastNode);
				return block;
			}
		}
		...
		// next interation.
		if (!this._NextNode)
			this._NextNode = (isLast || block == lastNode) ? null : FCKDomTools.GetNextSourceNode(block, true, null,
					lastNode);

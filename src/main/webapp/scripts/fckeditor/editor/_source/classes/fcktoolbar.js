...
// add by diyanliang 09-6-8 for 纵排toolbar
FCKToolbar.prototype.DucklingCreate = function(parentElement) {
	var oDoc = FCKTools.GetElementDocument(parentElement);
	var e = oDoc.createElement("table");
	e.className = "TB_Toolbar";
	e.style.styleFloat = e.style.cssFloat = (FCKLang.Dir == "ltr" ? "left"
	...
	for ( var i = 0; i < this.Items.length; i++) {
		if (this.Items[i] == "-") {
			var Break = parentElement.ownerDocument.createElement("div");
			Break.innerHTML = "";
			Break.style.clear = Break.style.cssFloat = FCKLang.Dir == "rtl" ? "right"
					: "left";
			parentElement.appendChild(Break);

			e = oDoc.createElement("table");
			e.className = "TB_Toolbar";
			e.style.styleFloat = e.style.cssFloat = (FCKLang.Dir == "ltr" ? "left"
					: "right");
			e.dir = FCKLang.Dir;
			e.cellPadding = 0;
			e.cellSpacing = 0;
			targetRow= e.insertRow(-1);
			if (!this.HideEnd) {
				eCell = targetRow.insertCell(-1);
				eCell.appendChild(oDoc.createElement("div")).className = "TB_End1111111111";
			}
			parentElement.appendChild(e);
		} else {
			this.Items[i].Create(targetRow.insertCell(-1));
	...
	if (!this.HideEnd) {
		eCell = targetRow.insertCell(-1);
		eCell.appendChild(oDoc.createElement("div")).className = "TB_End222";
	}
	parentElement.appendChild(e);
};

...
var FCKPanel = function(parentWindow) {
	...
	if (this._Window.createPopup) {
		...
		if (this._Window.document.body) {
			...
			//IE11
			this._Window.document.documentElement.appendChild(oIFrame);
		}
...
FCKPanel.prototype.ResizeForSubpanel = function(subpanel, width, height) {
	...
	if (this.IsRTL)
		x = x - w;
	var offsetWidth = this.ShowRect.w + w;
	var offsetHeight = Math.max(this.ShowRect.h, this.IncreasedY);
	if (this.ParentPanel)
		this.ParentPanel.ResizeForSubpanel(this, offsetWidth, offsetHeight);
	this._Popup.show(x, this.ShowRect.y, offsetWidth, offsetHeight, this.RelativeElement);
	return this.IsRTL;
};
FCKPanel.prototype._ShouldResize=function(){
	return (FCKBrowserInfo.IsIE7 || FCKBrowserInfo.IsIE9 || FCKBrowserInfo.IsIE10);
};
FCKPanel.prototype.Show = function(x, y, relElement, width, height) {
	...
	if (this._Popup) {
		...
		if (this._ShouldResize()) {
			if (this.ParentPanel
					&& this.ParentPanel.ResizeForSubpanel(this, iMainWidth,
							eMainNode.offsetHeight)) {
		...
		var oPos = FCKTools.GetDocumentPosition(this._Window, relElement.nodeType == 9 ? (FCKTools
				.IsStrictMode(relElement) ? relElement.documentElement : relElement.body) : relElement);
...
FCKPanel.prototype.Hide = function(ignoreOnHide, ignoreFocusManagerUnlock) {
	if (this._Popup){
		...
		//add by diyanliang 2011-6-24
		if (this.childPanel) {
			this.childPanel.todo = true;

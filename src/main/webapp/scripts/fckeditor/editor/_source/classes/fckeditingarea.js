	...
	if (this.Mode == FCK_EDITMODE_WYSIWYG) {
		...
		if (FCKBrowserInfo.IsIE)
			...
			if (oMatchBefore && oMatchAfter) {
				var sBody = html.substr(oMatchBefore[1].length, html.length - oMatchBefore[1].length
						- oMatchAfter[1].length); // This is the BODY tag contents.

				html = oMatchBefore[1] + // This is the HTML until the <body...> tag, inclusive.
		...
		if (FCK_IS_CUSTOM_DOMAIN && FCKBrowserInfo.IsIE) {
			...
			oIFrame.src = 'javascript:void( (function(){' + 'document.open() ;' + 'document.domain="' + document.domain
					+ '" ;' + 'document.write( window.parent._FCKHtmlToLoad );' + 'document.close() ;'
...
FCKEditingArea.prototype.MakeEditable = function() {
	...
	if (FCKBrowserInfo.IsIE) {
		...
		try {
			...
			oDoc.designMode = 'on';
			oDoc.body.id = (!FCKConfig.ducklingbodyid) ? "fckcontentbody" : FCKConfig.ducklingbodyid;

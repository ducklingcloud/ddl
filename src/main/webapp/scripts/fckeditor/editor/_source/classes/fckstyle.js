...
FCKStyle.prototype = {
		...
		if (range.CheckIsCollapsed()) {
			...
			for ( var i = 0; i < path.Elements.length; i++) {
							...
							for ( var att in styleAttribs) {
								if (FCKDomTools.HasAttribute(pathElement, att)) {
									...
									case 'class':
										...
										if (FCKDomTools.GetAttributeValue(pathElement, att) != this
				...
				isBoundary = isBoundary
						&& ((isBoundaryRight && !FCKDomTools.GetNextSibling(pathElement)) || (!isBoundaryRight && !FCKDomTools
	...
	CheckElementRemovable : function(element, fullMatch) {
		...
		if (elementName == this.Element) {
			...
			for ( var att in attribs) {
				...
				if (this._CompareAttributeValues(att, FCKDomTools.GetAttributeValue(element, att), (this
						.GetFinalAttributeValue(att) || ''))) {
					allMatched = true;
					if (!fullMatch)
						break;
	...
	_RemoveStylesFromElement : function(element) {
		...
		pattern = '(^|;)\\s*(' + pattern.replace(/\s*([^ ]+):.*?(;|$)/g, '$1|').replace(/\|$/, '') + '):[^;]+';
	...
	GetFinalAttributeValue : function(attName) {
		...
		attValue = attValue ? attValue[attName] : null;
	...
	_ApplyBlockStyle : function(range, selectIt, updateRange) {
		...
		while ((block = iterator.GetNextParagraph())) // Only one =
		{
			...
			// 使用“标题”格式时要保留原来外层标签中的属性 modified by zhangxiao 20110407
			newBlock = FCKDomTools.CopyAttributes(block, newBlock);
		...
		for ( var i = 0; i < convertedPreBlocks.length; i++) {
			...
			for ( var j = 0; j < currentBlock.childNodes.length; j++) {
				...
				if (cursor.nodeName.IEquals('br') && j != 0 && j != currentBlock.childNodes.length - 2
					...
					lastNewBlock = FCKDomTools.InsertAfterNode(lastNewBlock || currentBlock, doc
							.createElement(currentBlock.nodeName));
					continue;
				}
			...
			if (!nodeName || elementDTD[nodeName]) {
				...
				if ((FCK.DTD[currentNode.parentNode.nodeName.toLowerCase()] || FCK.DTD.span)[elementName]
					...
					if (nodeType != 1 || currentNode.childNodes.length == 0) {
						...
						// to the range.
						while (includedNode == parentNode.lastChild && elementDTD[parentNode.nodeName.toLowerCase()]) {
						...
						if (includedNode == includedNode.parentNode.lastChild
								&& !elementDTD[includedNode.parentNode.nodeName.toLowerCase()])
							applyStyle = true;
					} else {
	...
	_FixBookmarkStart : function(startNode) {
		...
		{
			if (startSibling.nodeType == 1 && FCKListsLib.InlineNonEmptyElements[startSibling.nodeName.toLowerCase()]) {

...
FCKIndentCommand.prototype = {
	Execute : function() {
		...
		// add by diyanliang 2008-11-05因为鼠标选中内容中如果包含LI就不能用和普通文本一样的方法缩进
		var test = FCKDomTools.GetCommonParentNode(range.StartNode || range.StartContainer, range.EndNode
				|| range.EndContainer, [ "ul", "ol" ]);
		if (test == null) {
			var diI = new FCKDomRangeIterator(range);
			// 解释下面代码
			// START @di_1@如果表格在页面的最开始,选中表格点缩进,会抛出b=null的异常,下面代码就是为了解决这个异常
			var testB = diI.Range.Clone();
			var showerrtype = 0;
			try {
				testB.Expand(testB.ForceBrBreak ? "list_contents" : "block_contents");
			} catch (e) {
				showerrtype = 1;
			}
			if (showerrtype == 1) {
			...
			// end @di_1@
			while ((diP = diI.GetNextParagraph())) {
				if (diP.parentNode.tagName.toLowerCase() == "ul") {
					alert("请选择正确的区域做此操作");
					return;
				} else if (diP.parentNode.tagName.toLowerCase() == "ol") {
					alert("请选择正确的区域做此操作");
					return;
				} else if (diP.parentNode.tagName.toLowerCase() == "tr") {
					alert("表格元素不允许做缩进操作")
					return;
		...
		var nearestListBlock = FCKDomTools.GetCommonParentNode(range.StartNode || range.StartContainer, range.EndNode
				|| range.EndContainer, [ 'ul', 'ol' ]);
		if (nearestListBlock)

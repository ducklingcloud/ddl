...
FCKSourceCommand.prototype.Execute = function() {
	// add by diyanliang 2009-4-22
	if (FCK.EditMode == 0) {
		var stbutton = [ "ok", "save", "preview" ];
		for (var j = 0; j < stbutton.length; j++) {
			var obutton = window.parent.document.getElementsByName(stbutton[j]);
			for (var i = 0; i < obutton.length; i++) {
				obutton[i].disabled = "disabled";
			}
		}
	} else {
		var stbutton = [ "ok", "save", "preview" ];
		for (var j = 0; j < stbutton.length; j++) {
			var obutton = window.parent.document.getElementsByName(stbutton[j]);
			for (var i = 0; i < obutton.length; i++) {
				obutton[i].removeAttribute("disabled");
			}
		}
	}
	// end
...
FCKCutCopyCommand.prototype = {
	...
	GetState : function() {
		...
		return FCK.EditMode != FCK_EDITMODE_WYSIWYG ? FCK_TRISTATE_DISABLED
...
/* dct 附件更新按钮状态刷新09-8-14 */
var FCKUpdateAttachCommand = function() {
	this.Name = "UpdateAttach";
};
FCKUpdateAttachCommand.prototype.Execute = function() {
	FCKDialog.OpenDialog("UpdateAttach", FCKLang.UpdateAttach,
			"dialog/UpdateAttach.jsp", 600, 260);
};
FCKUpdateAttachCommand.prototype.GetState = function() {
	if (FCK.EditMode != 0) {
		return -1;
	}
	var A = -1;
	...
	if (B) {
		if (B.getAttribute('href', 2)) {
			var href = B.getAttribute('href', 2);
...
var TableRedrawCommand = function() {
	this.Name = "TableRedraw";
};
TableRedrawCommand.prototype.Execute = function() {
	FCKDialog.OpenDialog("TableRedraw", FCKLang.TableRedraw,
			"dialog/fck_tableredraw.html", 715, 300);
};
TableRedrawCommand.prototype.GetState = function() {
	if (FCK.EditMode != 0) {
		return -1;
	}
	var A = -1;
	B = FCKSelection.HasAncestorNode("TABLE");
	if (B) {
		A = 0;
	}
	return A;

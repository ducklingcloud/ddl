...
FCKXHtml._AppendChildNodes = function(xmlNode, htmlNode, isBlockElement) {
	...
	if (isBlockElement && htmlNode.tagName
			...
			&& htmlNode.tagName.toLowerCase() != 'code') {
...
FCKXHtml._AppendNode = function(xmlNode, htmlNode) {
	...
	case 1:
		...
		if (FCKBrowserInfo.IsGecko && htmlNode.tagName.toLowerCase() == 'br'
		...
		if ( FCKBrowserInfo.IsGecko &&
			...
			if ( htmlNode.nextSibling || FCKBrowserInfo.IsIE11 )
				return false ;
		}
		...
		if (sNodeName == "st1:st1:chmetcnv") {
			return false;
		}
		var oNode = null;
		
		try{
			oNode = this.XML.createElement(sNodeName);
		}catch (exception){
			return false;
		}
	...
	case 3:
		if (htmlNode.parentNode && 
				(htmlNode.parentNode.nodeName.IEquals('pre') || htmlNode.parentNode.nodeName.IEquals('code')))
	...
	// Unknown Node type.
	default:
		xmlNode.appendChild(this.XML
				.createComment("Element not supported - Type: "
						+ htmlNode.nodeType + " Name: " + htmlNode.nodeName));
		break;
	}
	return true;
}

...
var FCK = {
	...
	FixBody : function() {
		...
		while (oNode) {
			...
			case 1:
				...
				if (!FCKListsLib.BlockElements[nodeName] && nodeName != 'li'
	...
	GetData : function(format) {
		...
		// add by diyanliang
		if (!oDoc) {
			if (window.document.getElementById("xEditingArea").firstChild.contentWindow.document)
				oDoc = window.document.getElementById("xEditingArea").firstChild.contentWindow.document;
		}
		// end
		if (!oDoc)
			return null;
		...
		var data = FCK.DataProcessor.ConvertToDataFormat(
				isFullPage ? oDoc.documentElement : oDoc.body, !isFullPage,
		...
		// add by diyanliang for WordPaste FixDom 09-6-19
		if (format == 'FixDom') {

			if (!FCK.EditorDocument) {
				var html = FCKDomTools.FixDomTree(window.document
						.getElementById("xEditingArea").firstChild.contentWindow.document.body);
				var newbody = FCK.EditorDocument.createElement("body");
				newbody.innerHTML = html;
				var rehtml = FCKXHtml.GetXHTML(newbody);
				return rehtml;
			} else {
				var html = FCKDomTools.FixDomTree(FCK.EditorDocument.body);
				var newbody = FCK.EditorDocument.createElement("body");
				newbody.innerHTML = html;
				var rehtml = FCKXHtml.GetXHTML(newbody);
				return rehtml;
	...
	SetData : function(data, resetIsDirty) {
		...
		if (FCKBrowserInfo.IsIE && FCK.EditorDocument && !FCKBrowserInfo.IsGeckoLike) {
			FCKTools.RemoveEventListener(FCK.EditorDocument, "selectionchange", Doc_OnSelectionChange);
		}
		...
		if (FCK.EditMode == FCK_EDITMODE_WYSIWYG) {
			...
			// add by diyanliang 2011-4-7 start
			if (FCKBrowserInfo.IsIE){
				if (!FCKBrowserInfo.IsGeckoLike){//IE11 not replace.
					data = data.replace(new RegExp(">&nbsp;</td>", "gm"), "></td>");
					data = data.replace(/>[\s]<\/td>/ig, "></td>");
				}
			}else{
				data=data.replace(new RegExp(">&nbsp;</td>","gm"),"><br type=\"_moz\"></td>");
				data=data.replace(/>[\s]<\/td>/ig,"><br type=\"_moz\"></td>");
			}
			// end
	...
	GetNamedCommandState : function(commandName) {
		try {
			...
			if (!FCK.EditorDocument.queryCommandEnabled(commandName))
				...
				return FCK.EditorDocument.queryCommandState(commandName) ? FCK_TRISTATE_ON
						: FCK_TRISTATE_OFF;
			}
		} catch (e) {
			return FCK_TRISTATE_OFF;
	...
	Paste : function(_callListenersOnly) {
		if (_callListenersOnly == false) {
			return;
		}

		return FCK._ExecPaste();
	},
	...
	ReDrawTable : function(table, classname) {
		if (classname)
			table.className = classname;
		var tablecsstext = table.style.cssText.replace(new RegExp("background",
				"gm"), "");
		tablecsstext = tablecsstext.replace(new RegExp("BACKGROUND", "gm"), "");
		tablecsstext = tablecsstext.replace(new RegExp("border", "gm"), "");
		tablecsstext = tablecsstext.replace(new RegExp("BORDER", "gm"), "");
		table.style.cssText = tablecsstext;
		var arr = FCKTableHandler._CreateTableMap(table);
		for (var i = 0; i < arr.length; i++) {
			// 偶行
			if ((i + 1) % 2 == 0) {
				for (var j = 0; j < arr[i].length; j++) {
					arr[i][j].className = "evener";
					arr[i][0].className = "lefterevener";
					var csstext = arr[i][j].style.cssText.replace(new RegExp(
							"background", "gm"), "");
					csstext = csstext.replace(new RegExp("BACKGROUND", "gm"),
							"");
					csstext = csstext.replace(new RegExp("border", "gm"), "");
					csstext = csstext.replace(new RegExp("BORDER", "gm"), "");
					arr[i][j].style.cssText = csstext;
				}
			} else {
				for (j = 0; j < arr[i].length; j++) {
					arr[i][j].className = "odder";
					arr[i][0].className = "lefterodder";
					var csstext = arr[i][j].style.cssText.replace(new RegExp(
							"background", "gm"), "");
					csstext = csstext.replace(new RegExp("BACKGROUND", "gm"),
							"");
					csstext = csstext.replace(new RegExp("border", "gm"), "");
					csstext = csstext.replace(new RegExp("BORDER", "gm"), "");
					arr[i][j].style.cssText = "";
				}
			}
		}
		for (j = 0; j < arr[0].length; j++) {
			arr[0][j].className = "header";
		}
	},
	ReDrawTableSaveUndo : function() {
		FCKUndo.SaveUndoStep();
	},
	ReDrawTableRedo : function() {
		FCKUndo.Undo();
	}
};
...
function _FCK_ProtectEvents_ReplaceTags(tagMatch) {
	// edit by diyanliang 为了保留onclick事件 09-11-30
		...
		if (parentElement && parentElement.nodeName.IEquals('body')) {
			if (FCK.EditorDocument.body.childNodes.length == 1
				...
				if (!paddingNode.childNodes.firstChild) {
					paddingNode.appendChild(FCKTools.GetElementDocument(
							paddingNode).createTextNode('\ufeff'));
					clearContents = true;
				}
...
function _FCK_GetEditorAreaStyleTags() {
	return FCKTools.GetStyleHtml(FCKConfig.EditorAreaCSS)
			+ FCKTools.GetStyleHtml(FCKConfig.EditorAreaDucklingCSS)
...
var FCKFocusManager = FCK.FocusManager = {
	...
	AddWindow : function(win, sendToEditingArea) {
		...
		if (FCKBrowserInfo.IsIE)
			oTarget = win.nodeType == 1 ? win
					: win.frameElement ? win.frameElement : win.document;
		...
		FCKTools.AddEventListener(oTarget, 'focus',
				sendToEditingArea ? FCKFocusManager_Win_OnFocus_Area

...
function Doc_OnPaste() {
	...
	FCKTools.removeEventListener(body, 'paste', Doc_OnPaste);

	var ret = FCK.Paste(!FCKConfig.ForcePasteAsPlainText
			&& !FCKConfig.AutoDetectPasteFromWord);
	FCKTools.addEventListener(body, 'paste', Doc_OnPaste);
...
FCK.InitializeBehaviors = function(dontReturn) {
	...
	FCKTools.AddEventListener(this.EditorDocument, 'mouseup', Doc_OnMouseUp);

	// Intercept pasting operations
	FCKTools.AddEventListener(this.EditorDocument.body, 'paste', Doc_OnPaste);


	// Intercept drop operations
	FCKTools.AddEventListener(this.EditorDocument.body, 'drop', Doc_OnDrop);

	// Reset the context menu.
	FCK.ContextMenu._InnerContextMenu.AttachToElement(FCK.EditorDocument.body);

	FCKTools.AddEventListener(this.EditorDocument, 'keydown', FCK._KeyDownListener);

	FCKTools.AddEventListener(this.EditorDocument, 'dblclick', Doc_OnDblClick);

	// Catch cursor selection changes.
	FCKTools.AddEventListener(this.EditorDocument, 'selectionchange', Doc_OnSelectionChange);
...
FCK._ExecPaste = function() {
	...
	if (sHTML === false)
		FCKTools.RunFunction(FCKDialog.OpenDialog, FCKDialog, [
				'FCKDialog_Paste', FCKLang.Paste, 'dialog/fck_paste.html', 400,
				330, 'Security' ]);
	else {
...
FCK._CheckIsPastingEnabled = function(returnContents) {
	...
	FCKTools.RemoveEventListener(document.body,'paste',FCK_CheckPasting_Listener);

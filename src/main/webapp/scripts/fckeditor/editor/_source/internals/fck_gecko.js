...
FCK.InitializeBehaviors = function() {
	...
	this._ExecCheckCaret = function(evt) {
		...
		var moveCursor = function() {
			...
			if (!(parentTag == 'a' || (!FCKBrowserInfo.IsOpera && String(node.parentNode.contentEditable) == 'false') || (!(FCKListsLib.BlockElements[parentTag] || FCKListsLib.NonEmptyBlockElements[parentTag]) && keyCode == 35)))
				return;
			...
			if (nextTextNode) {
				...
				while (node.parentNode
						...
						&& (!FCKListsLib.BlockElements[node.parentNode.tagName.toLowerCase()] && !FCKListsLib.NonEmptyBlockElements[node.parentNode.tagName
								.toLowerCase()]))
					node = node.parentNode;

				if (FCKListsLib.BlockElements[parentTag] || FCKListsLib.EmptyElements[parentTag]
...
FCK.InsertHtml = function(html) {
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		html = '<span id="__fakeFCKRemove1__" style="display:none;">fakeFCKRemove</span>' + html
				+ '<span id="__fakeFCKRemove2__" style="display:none;">fakeFCKRemove</span>';
	}
...
FCK.PasteAsPlainText = function() {
	...
	FCKTools.RunFunction(FCKDialog.OpenDialog, FCKDialog, [ 'FCKDialog_Paste', FCKLang.PasteAsText,
...
FCK.CreateLink = function(url, noUndo) {
	...
	if (url.length > 0) {
		...
		var oLinksInteractor = this.EditorDocument.evaluate("//a[@href='" + sTempUrl + "']", this.EditorDocument.body,
...
FCK.InsertPlugin = function(A) {
	...
	FCKUndo.SaveUndoStep();
	if (FCKBrowserInfo.IsGecko) {
		A = "<span id=\"__fakeFCKRemove1__\" style=\"display:none;\">fakeFCKRemove</span>" + A
				+ "<span id=\"__fakeFCKRemove2__\" style=\"display:none;\">fakeFCKRemove</span>";
	}
	try {
	...
	if (FCKBrowserInfo.IsGecko) {
		FCKDomTools.RemoveNode(B.getElementById("__fakeFCKRemove1__"));
		FCKDomTools.RemoveNode(B.getElementById("__fakeFCKRemove2__"));
	}
	this.Focus();

...
var FCKDomTools = {
	RemoveAttr:function(target, attrName){
		if (target){
			if (attrName){
				if (FCKBrowserInfo.IsGeckoLike || FCKBrowserInfo.IsGecko){
		...
		try {
			if (paddingNode.parentNode != doc.body
					...
					|| (paddingNode.firstChild
							&& paddingNode.firstChild.nodeValue != '\xa0' && String(
							paddingNode.firstChild.tagName).toLowerCase() != 'br')) {
	...
	ArrayToList : function(listArray, markerObj, baseIndex) {
		...
		while (true) {
			...
			if (item.indent == indentLevel) {
				...
				//add by diyanliang 2011-4
				currentListItem = this.CopyAttributes(item.m_element, currentListItem);
				rootNode.appendChild(currentListItem);
				for ( var i = 0; i < item.contents.length; i++){
					currentListItem.appendChild(item.contents[i]
							.cloneNode(true));
					if (item.id != "")
						currentListItem.id = item.id;
					currentListItem.className = item.className;
				}
				...
				if (FCKBrowserInfo.IsGeckoLike && !FCKBrowserInfo.IsIE11
	...
	CopyAttributes:function(fromEl ,toEl){
		var s_attributes=fromEl.attributes;
		for(var i=0;i<s_attributes.length;i++){
			...
			if(attribute.specified){
				var	attrValue = fromEl.getAttribute( attrName );
				if(toEl.nodeType==1)
					toEl.setAttribute( attrName, attrValue );
			}
	...
	FixDomTree:function (element){
		var strip=["P:COLORSCHEME"];
		var ignore=["META", "LINK","STYLE","FCK:META","TITLE","O:SHAPELAYOUT","O:IDMAP","COL","O:P"];
		...
		if (element.nodeType == 3) {
			var strtext=element.nodeValue;
			strtext=strtext.replace(new RegExp("&","gm"),"&amp;");
			strtext=strtext.replace(new RegExp(">","gm"),"&gt;");
			strtext=strtext.replace(new RegExp("<","gm"),"&lt;");
			str = str+strtext;
			...
			for (var i=0;i<ignore.length;i++){
				if(element.scopeName){
					if (ignore[i]==(element.scopeName+":"+element.nodeName).toUpperCase()){
							return "";
						}
				}else{
					if (ignore[i]==element.nodeName.toUpperCase()){
							return "";
						}
				}
			}
			...
			{
				if(element.scopeName){
					if (strip[i]==(element.scopeName+":"+element.nodeName).toUpperCase()){
							stripped=true;
						}
				}else{
					if (strip[i]==element.nodeName.toUpperCase()){
							stripped=true;
						}
				}
			}
			if (!stripped){
				if (element.tagName == "IMG" && element.getAttribute("_fckflash")) {
						element = FCK.GetRealElement(element);
				}
				//因为ie下对a元素的href做了处理
				if(element.tagName=="A"){
					if(element.href){
						var strhref=element.getAttribute('href', 2);
						element._fcksavedurl=strhref;
					}
				...
				if (a != null) {
					...
					for (i = 0; i < a.length; i++) {
						if (a[i].specified) {
							 if(a[i].name=="href"){
								var attValue=a[i].value;
								if(attValue){
									attValue=attValue.replace("&quot;","");
									attValue=attValue.replace("\"","");
								}
								str += " href=\"" + element.getAttribute('href', 2) + "\"";
							}else if( a[i].name.indexOf(":")==-1){
								var attValue=a[i].value;
								if(attValue){
									attValue=attValue.replace("&quot;","");
									attValue=attValue.replace("\"","");
								}
								str += " " + a[i].name + "=\"" + attValue + "\"";
							}	
						}
						if(a[i].name=="style"){
							noStyle=false;
					...
					if(element.style.cssText && noStyle){
						str += " " + "style" + "=\"" + element.style.cssText + "\"";
					}
				}
				
				if(element.tagName.toLowerCase()=="input"){
					str=str+"value=\""+element.getAttribute("value")+"\"";
				}
				
				if(!!/^(area|base|basefont|col|frame|hr|img|br|input|isindex|link|meta|param)$/.test(element.tagName.toLowerCase())){
					return str+" />";
				}
			...
			if (childnodes != null) {
				for (var i = 0; i < childnodes.length; i++) {
					str = str + this.FixDomTree(childnodes[i]);
				}
			}
			if (!stripped)
			{
				str= str + "</" + element.tagName + ">";
			}

		}

		//这步替换是不得已而为之 因为在word和excel同时打开时，浏览器的dom会在td之间加span
		str=str.replace("<SPAN style=\"\"></SPAN>","");
		return str;
	}
};
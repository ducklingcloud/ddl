...
function FCK_ContextMenu_GetListener(listenerName) {
	try {
		return E2ContextMenu(listenerName);
	} catch (e) {
			...
			AddItems : function(menu, tag, tagName) {
				...
				if (bIsCell) {
					...
					if (FCKBrowserInfo.IsGecko)
						oItem.AddItem('TableMergeCells', FCKLang.MergeCells,
								60, FCKCommands.GetCommand('TableMergeCells')
										.GetState() == FCK_TRISTATE_DISABLED);
					else {
						oItem.AddItem('TableMergeRight', FCKLang.MergeRight,
								60, FCKCommands.GetCommand('TableMergeRight')
										.GetState() == FCK_TRISTATE_DISABLED);
					...
					oItem.AddItem('TableInsertColumnBefore',
							FCKLang.InsertColumnBefore, 64);
					oItem.AddItem('TableInsertColumnAfter',
							FCKLang.InsertColumnAfter, 64);
					oItem.AddItem('TableDeleteColumns', FCKLang.DeleteColumns,
							65);
				}
	...
	case "Link":
		...
		// add by diyanliang 09-4-24
	case "DMLPlugin":
		return {
			AddItems : function(menu, tag, tagName) {
				if (tagName == "IMG"
						&& (tag.className).toUpperCase() == "PLUGIN") {
					var title = tag.title;
					if (title.indexOf("DEMapPlugin") != -1) {
						menu.AddSeparator();
						menu.AddItem("DEMap", FCKLang.DEMap, 74);

					} else if (title.indexOf("ImageScrollPlugin") != -1) {
						menu.AddSeparator();
						menu.AddItem("ImageScroll", FCKLang.ImageScroll, 73);

					} else if (title.indexOf("DmlResultsetPlugin") != -1
							|| title.indexOf("DmlFieldPlugin") != -1) {
						// do nothing

					} else {
						menu.AddSeparator();
						menu.AddItem("DMLPlugin", FCKLang.DMLPluginMsg, 52);
					}
				}
			}
		};
	...
	case "Anchor":
		return {
			AddItems : function(menu, tag, tagName) {
				...
				if (bIsAnchor
						|| (tagName == 'IMG' && tag.getAttribute('_fckanchor'))) {
					menu.AddSeparator();
		...
		return {
			AddItems : function(menu, tag, tagName) {
				if (tagName == 'INPUT'
					...
					menu.AddSeparator();
					menu.AddItem("DMLButton", FCKLang.ButtonProp, 54);
				}
			}
		};
	...
	case "Select":
		return {
			AddItems : function(menu, tag, tagName) {
				if (tagName == 'SELECT') {
					menu.AddSeparator();
					menu.AddItem("DMLSelect", FCKLang.SelectionFieldProp, 53);
				}
			}
		};
	...
	case "UpdateAttach":
		return {
			AddItems : function(menu, tag, tagName) {
				var isAnchor = (tagName == "A" || FCKSelection.HasAncestorNode("A"));
				if (isAnchor) {
					var node = FCKSelection.MoveToAncestorNode("A");
					if (node.getAttribute('href', 2)) {
						var href = node.getAttribute('href', 2);
						if (href.indexOf("attach/") != -1) {
							menu.AddSeparator();
							menu.AddItem("UpdateAttach", FCKLang.UpdateAttach,
									59);
						}
					}
				}

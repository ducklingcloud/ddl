...
//dml_data里面检查同一个form中是否有同名控件,存在返回true，不存在返回false
function dmlDataCheckName(dName,obj){
	var childnodes = obj.childNodes;
	if (childnodes != null) {
		for (var i = 0; i < childnodes.length; i++) {
			if(childnodes[i].nodeType==1){
				var dmldesc=childnodes[i].getAttribute("dmldesc")
				if(dmldesc!=null){
					if(dmldesc==dName){
						return true;
					}
				}
				var getre=dmlDataCheckName(dName,childnodes[i])
				if(getre==true){
					return true
				}
			}
...
//dml_data里面检查同一个form中是否有同名不是radio的控件,存在返回true，不存在返回false
function dmlDataCheckNameNotRadio(dName,obj){
	var childnodes = obj.childNodes;
	if (childnodes != null) {
		for (var i = 0; i < childnodes.length; i++) {
			if(childnodes[i].nodeType==1){
				var eType=childnodes[i].type;
				if(eType){
				...
				var getre=dmlDataCheckNameNotRadio(dName,childnodes[i])
				if(getre==true){
					return true
				}
			}
...
//dml_data里面检查同一个form中是否有同名不是CHECKBOX的控件,存在返回true，不存在返回false
function dmlDataCheckNameNotCheckbox(dName,obj){
	var childnodes = obj.childNodes;
	if (childnodes != null) {
		for (var i = 0; i < childnodes.length; i++) {
			if(childnodes[i].nodeType==1){
				...
				var getre=dmlDataCheckNameNotCheckbox(dName,childnodes[i])
				if(getre==true){
					return true
				}
			}

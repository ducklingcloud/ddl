...
//					FCKSelection.SelectNode(tableNode);
//				else{
//					
//				
////					rbCell.focus();
...
//				}
//				return null
//				lxDist = mouse.x - (FCKTools.GetWindowPosition( w, tableNode ).x+tableNode.clientWidth );
//				if ( lxDist < 0 && minRxDist < 0 && minRxDist < -2 )
//					return null ;
//				if ( lxDist > 0 && minRxDist > 0 && lxDist > 3 )
//					return null ;
//				
//				return { "leftCell" : rbCell, "rightCell" :null  } ;
//			}else
//				return null ;
	...
	"_ResizeBarMouseDownListener" : function( evt )
	{
		...
		FCKDomTools.SetOpacity(FCKDragTableHandler._ResizeBar, 0.5);
	...
	"_ResizeBarMouseUpListener" : function( evt )
	{
		...
		for ( var i = 0 ; i < processedCells.length ; i++ ){
			FCKDomTools.RemoveAttr(processedCells[i],'_Processed');
		}
	...
	"_MouseFindHandler" : function( FCK, evt )
	{
		...
		if ( this._ResizeBar && this._LeftCell )
		{
			...
			if(this._RightCell){
				...
				var leftPos = FCKTools.GetWindowPosition( FCK.EditorWindow, this._LeftCell ) ;
				var rightPos = FCKTools.GetWindowPosition( FCK.EditorWindow, FCKTools.GetElementAscensor( this._LeftCell, "table" ) ) ;
				...
				{
					this._HideResizeBar() ;
				var otable=	FCKTools.GetElementAscensor( this._LeftCell, "table" )
	...
	"_ShowResizeBar" : function( w, table, mouse )
	{
		if ( this._ResizeBar == null )
		{
			...
			FCKDomTools.SetOpacity(paddingBar, 0.1);
		...
		if ( isNaN( cs ) )
			cs = 0 ;
		var barWidth = Math.max( bw+100, cs+100 ,3) ;
		var paddingStyles =
		{
			'top'		: barTop + 'px',
			'height'	: barHeight + 'px',

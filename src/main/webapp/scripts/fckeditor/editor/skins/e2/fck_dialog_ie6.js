...
(function() {
	...
	var fixSizes = window.DoResizeFixes = function() {
		...
		for ( var i = 0; i < fckDlg.childNodes.length; i++) {
			...
			case 'blocker':
			case 'cover':
				child.style.width = Math.max(0,
						fckDlg.offsetWidth - 16 - 16 + 4); // -left -right + 4
				child.style.height = Math.max(0,
						fckDlg.clientHeight - 20 - 2 + 4); // -bottom -top + 4
				break;
	...
	onLoad = function() {
		fixSizes();
		fixCloseButton();
		FCKTools.AddEventListener(window, 'resize', fixSizes, false);
		FCKTools.RemoveEventListener(window, 'load', onLoad);
	};
	FCKTools.AddEventListener(window, 'load', onLoad, false);
})();

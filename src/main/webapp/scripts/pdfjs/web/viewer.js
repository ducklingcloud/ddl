...
var PresentationMode = {
  ...
  request: function presentationModeRequest() {
	//custom 自定义最大化
    var ww=parent.innerWidth || parent.document.documentElement.clientWidth || parent.document.body.clientWidth;
    var wh=parent.innerHeight || parent.document.documentElement.clientHeight || parent.document.body.clientHeight;
	var pmBtn = document.getElementById("presentationMode");
	var vw = PDFJS.viewerWrapper;
	if(WRAPPER_WIDTH ==0){
		WRAPPER_WIDTH = vw.style.width;
		WRAPPER_HEIGHT = vw.style.height;
	}
	if(pmBtn.className=="toolbarButton presentationMode hiddenSmallView"){
		vw.style.width = (ww-5) + "px";
		vw.style.height = (wh-5) + "px";
		vw.style.position="fixed";
		vw.style.top = 0;
		vw.style.left = 0;
		pmBtn.className = "toolbarButton presentationModeOff hiddenSmallView";
	}else{
		vw.style.position="static";
		vw.style.width = WRAPPER_WIDTH;
		vw.style.height = WRAPPER_HEIGHT;
		pmBtn.className = "toolbarButton presentationMode hiddenSmallView";
	}
...
function webViewerInitialized() {
  ...
  var file = 'file' in params ? params.file : PDFJS.defaultUrl;
    ...
    switch (evt.keyCode) {
      ...
      case 27: // esc key
        ...
        if (!PDFView.supportsIntegratedFind && PDFView.findBar.opened) {
          ...
          handled = true;
        }else if(document.getElementById("presentationMode").className=="toolbarButton presentationModeOff hiddenSmallView"){

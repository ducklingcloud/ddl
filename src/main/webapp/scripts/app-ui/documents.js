...
window.featureManual={
	init:function(){
		...
		$('#resourceList .editFileName .updateFolder').die().live('click',function(){
			...
			if(featureManual.renderData(span).itemType=='DFile'){
				...
				if(f.lastIndexOf(".")>-1){
					var sub = f.substring(f.lastIndexOf(".")+1);
					if(sub.toLowerCase()=='dsf'){
						var last = f.substring(0,f.lastIndexOf(".")).lastIndexOf(".");
						if(last>-1){
							fileName = fileName+f.substring(last);
						}else{
							fileName +=".dsf";
						}
					}else{
		...
		$("a.down-file").die().live('click',function(){
			...
			if(item.itemType=='DFile'){
				location.href = featureManual.option.teamHome+"/downloadResource/"+item.rid+"?imageType=original";
			}else if(item.itemType=='Folder'){
				location.href = featureManual.option.teamHome+"/downloadResource/123?rids[]="+item.rid+"&func=folder";
			}
		});
	...
	updateTotalSize:function(method,type,num){
		...
		if(type=='add'){
			...
			$('#totalSize span').attr('title',s+getNumName());
		}else if(type=='delete'){
			s = s-num;
			$('#totalSize').attr("size",s);
			$('#totalSize span').html(" ("+s+")");
	...
	notice_handler:{
  			...
  			noMatch: function() {
  				...
  				if(queryType=='myStarFiles'){
  					$('#notice').addClass('large').text('当前没有星标文件').show();
  				}else if(queryType=='myCreate'){
  					$('#notice').addClass('large').html('<p class="emptyFolder"></p>您还没有创建文档或上传文件<p class="dragHint">点击上传按钮或将文件拖放到此窗口即可上传</p>').show();;
  					/*$('#no-result-helper').removeAttr('style');*/
  				}else if(queryType=='myRecentFiles'){
  					$('#notice').addClass('large').text('当前没有常用文件').show();
  				}else if(queryType=='tagQuery'){
  					$('#notice').addClass('large').text('当前标签下没有文件').show();
  				}else{
  					if(featureManual.currentDirRid>0){
  						...
  						$('#notice').addClass('large').html('<p class="emptyFolder"></p>该团队还未创建协作文档或上传文件<p class="dragHint">您可以点击上传按钮或将文件拖放到此窗口即可上传</p>').show();
  						/*$('#no-result-helper').removeAttr('style');*/
  					}
  				}
  			},
...
//				$.tmpl('fileItemTemp', data).appendTo(v);
			...
			moveFile:function(data){
				...
				window.featureManual.updateTotalSize('deleteFolder','delete',data.length);
			...
			renderNavBar:function(d){
				...
				if(d){
					$(v).append("<li id='totalSize' size='"+size+"' ><span style='color:#999' title='"+size+getNumName()+"'>&nbsp;("+size+")</span></li>");
				}
...
function getNumName(){
	var queryType = getQueryType();
	if(queryType=='myCreate'||queryType=='myRecentFiles'||queryType=='tagQuery'||queryType=='teamRecentChange'){
		return "个文件";
	}else{
		return "个文件/文件夹";
	}
...
function showDownloadBtn(){
	...
	for(var i = 0;i<checked.length;i++){
		var d = featureManual.renderData(checked[i]);
		if(d.itemType=='DFile'||d.itemType=='Folder'){
			show = true;
		}
	}
	if(show){
		$(".selectedOperGroup #downLoadSelected").removeClass("disableBtn");
	}else{
		$(".selectedOperGroup #downLoadSelected").addClass("disableBtn");
	}
}
...
function replaceHeadImg(item){
	 ...
	 var getImageStatus=window.featureManual.option.teamHome+"/downloadResource/"+dataTemp.rid+"?func=getImageStatus&type=small";
	 var downloadUrl=window.featureManual.option.teamHome+"/downloadResource/"+dataTemp.rid+"?type=doc&imageType=fixSmall";
	 if(getViewModel()=='grid'){
...
function assembleImage(res){
	var img = {};
	img.image = site.getURL("download",res.rid + "?type=doc");
	img.thumb = site.getURL("download",res.rid + "?type=doc&imageType=fixSmall");
...
$(document).ready(function(){
	...
	$("#downLoadSelected:not(.disableBtn)").live('click',function(){
		var items = $("div.oper .showSelectedOperate:checked");
		if(!items){
			return ;
		}
		...
		for(var i=0;i<items.length;i++){
			var d = window.featureManual.renderData(items[i]);
			if(d.itemType=='DFile'||d.itemType=='Folder'){
				params+="rids[]="+d.rid+"&";
			}
		}
		params+="func=folder";
		location.href = window.featureManual.option.teamHome+"/downloadResource/123?"+params;
	});
	...
	function loadBrowserTree(teamCode){
		var url = (teamCode ? site.getJSPURL(teamCode) : actionURL.teamHome) + "/fileManager";
		$("#file_browser").jstree(
				{
					"json_data" : {
						"ajax" : {
	...
	function isSelectedCurrentTeam(){
		return actionURL.teamHome === site.getJSPURL($("#teamSel").val());
	}
							...
							dataType:"json",
						   	success: function(data){
						   		...
						   		if(data.type=='meepoCopy'){
									   ...
									   var d = new PipeCopy({"showTable":"pipeShowTable","taskId":data.taskId,"url":file_manager_url,"selectedCount":originalRids.length,
	...
	var index = 0;
     ...
     var topUploader = new qq.FileUploader({
         ...
         onComplete:function(id, fileName, data){
        	...
        	if(data.success){
        		featureManual.renderPrePendFile(data.resource,'uploadFile');
        	}
     ...
     $(".share-file").die().live("click",function(){
    	 var ver = $(this).parents("li.files-item");
    	 if(ver[0]==undefined){
    		 return false;
    	 }
    	 shareFile(window.featureManual.renderData($(this)));
     });
     ...
     $(".share-Resource").die().live('click',function(){
 	...
 	var deleteAlertNotice={
 		...
 		deleteSeleted:function(rids){
			...
			$.getJSON(window.featureManual.option.oprateUrl+"?func=deleteResources",o,function(data){
				if(data.result){
					deleteAlertNotice.success(rids.length);
	...
	Galleria.ready(function() {
		...
		that.bind("loadstart", function(e) {
	        ...
	        gb.find("a.magnifier").attr("href", site.getURL("originalImage", gd.rid));
	        gb.find("a.down-file").attr("href", site.getURL("download", gd.rid) + "?type=doc&imageType=original");
	        gb.find("li.share-file").off("click").on("click",function(){
	        	shareFile(gd);
	        });
	        gb.find("li.picture-share-resource").off("click").on("click",function(){
	        	 var team = actionURL.teamHome;
	        	prepareShareResource(team+"/shareResource",gd.rid,gd.fileName,"opareteFileMessage");
	        });
...
//		resetSort();
		...
		$("div.filterBoard input[name=search_input]").val("在当前列表中查找");
	...
	// intro steps begin
		...
		/*个人空间共6步，不管是不是拥有管理权限。步骤0，点击管理员、文件动态、新建文件夹上传文件、分享历史之后置为3或者4
		 *第5步是下载客户端、拖拽和分享文件（现在已经去掉）
		 *现在把第5步内容替换为送空间活动，并从5改为6，所以数据库中的数据会出现 0 3 4 5 6，没有其他值。*/
		$.ajax({
			...
			success:function(data){
				...
				if(step==3 || step==4 || step==5){
					$(".closeGuide").hide();
					/*如果是3、4、5，说明已经把管理权限、文件动态、新建文件夹上传文件、分享看完了，所以要直接看广告了*/
					$("#mask_common_1,#guide_prev,#guide_next,#intro_common_4").hide();
					return;
				}
				
				/*如果能走到这一步，应该就是step=0的吧*/
				if(step < totalStep) {
					...
					if ($("#teamConfig").length > 0) {
						...
						$("#intro_common_1,#guide_next,.closeGuide").show();
						$("#intro_common_2").hide();
					}else {
						$("#mask_common_1").show();
						$("#intro_common_2,#guide_next,.closeGuide").show();
					}
					// view the unmanaged team before managed team.vera
					UI.controlPrevAndNext();
				}
			},
			error:function(){
				step = 0;
		...
		/*vera added on 2014.7.4 begin*/
		var UI = {
			controlPrevAndNext:function(){
				var curr=$('div.intro_step:visible').attr("id");
				var first="";
				if ($("#teamConfig").length > 0) {
					first="intro_common_1";
				}
				else {
					first="intro_common_2";
				}
				var last="intro_common_4";
				if(curr==first){
					$('#guide_prev').hide();
					$('#guide_next,.closeGuide').show();
				}else if(curr==last){
					$('#guide_prev,.closeGuide').show();
					$('#guide_next').hide();
				}else{
					$('#guide_prev,#guide_next,.closeGuide').show();
				}
			}
		}
		
		$("#guide_prev").click(function(){
			var INDEX = parseInt($('div.intro_step:visible').attr("id").substring(13,14)) - 1;
			$('div.intro_step:visible').fadeOut(500,function(){
				$('div#intro_common_'+INDEX).fadeIn(500);
				UI.controlPrevAndNext();
			});
		});
		...
		$(".closeGuide").click(function(){
			$(this).hide();
			$("#mask_common_1,#guide_prev,#guide_next,div.intro_step").hide();
			$(".isHighLight").removeClass("isHighLight");
			step =4;
			postStep(step);
		});
		/*vera added on 2014.7.4 end*/
		 ...
		 $("div.filterBoard input[name=search_input]").on('input propertychange', function() {
		 	...
		 	if(!window.featureManual.isLoading){
				//window.setTimeout(function(){
			    	...
			    	buildHash(arr);
				//}, 500);
				//window.featureManual.isLoading = true;
			}
		});
});
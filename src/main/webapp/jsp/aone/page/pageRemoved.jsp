...
<div class="ui-wrap">
	<div class="error-center">
		<c:choose>
			<c:when test="${adminUnpublish }">
				<h3>该版本用户未发布！</h3>
				<hr/>
				<a href="<vwb:Link context='teamHome' format='url'/>">跳转到团队首页</a>
			</c:when>
			<c:otherwise>
				<h3>对不起，您请求的页面已被删除！</h3>
				<hr/>
				<c:choose>
					<c:when test="${recoverFlag}">
						<p style="line-height: 1.7em;font-size: 13px;color: #333">我们为您做了备份，<a class="recoverPage">
...
$(document).ready(function(){
	$(".recoverPage").live('click',function(){
		var aa= window.confirm("您确定要恢复此页面吗？\n恢复后，团队的其他成员可以看到该文档");
		if(aa){
			var rid = $(this).find("input[name='rid']").val();
			var url = site.getURL("teamHome");
			var params ={"func":"recoverResource","rid":rid,"itemType":"DPage"};
			$.ajax({
				url:url,
				type:'post',
				data:params,

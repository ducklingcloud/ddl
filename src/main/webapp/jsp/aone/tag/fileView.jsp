 ...
 $(function(){
	 ...
	 $.ajax({
		 ...
		 $.each(ids,function(i,n){
			 ...
			 var downloadStr = res.itemType =='DFile' ?
                                           "<a style=\"float:left;margin-left:5px;\" href=\"<vwb:Link context='download' format='url'/>"+ n +"?type=doc&imageType=original\"> <i class=\"icon-download-alt\"></i></a>" :
                                           "";
			 $("#exist-attach-list").append(
                             "<li><span class=\"headImg "+ res.itemType +" "+
                             res.fileType +"\"></span><a class=\"file ellipsis\" rid=\""+
                             n +"\" href=\"${teamHome}/r/"+ n +"\" title='"+
                             res.fileName +"'>"+ res.fileName +"</a>"+
                             downloadStr +" <a class=\"lightDel\" rid=\""+ n +"\"> </a></li>"
                         );
		     }
		 });
	     },
...
<script type="text/javascript">
 ...
 $(function(){
	 ...
	 prepareShareResource("<vwb:Link context='team' format='url'/>/shareResource",
                              "${resource.rid}",
                              "${resource.title}",
                              "opareteFileMessage");
     });
	 ...
	 if(checkHtml5()||isIE9()){
	     $("#viewerWrapper").append(
                 "<iframe src=\"<vwb:Link page='${resource.rid}' context='f' format='url'/>"+
                 "?func=onlineViewer&version=${curVersion.version}\""+
                 " height=\"100%\" width=\"100%\" scrolling=\"no\" >"+
                 "</iframe>");
	 }else{
	     $("#fileInfo").empty();
	     $("#fileInfo").append(
                 "<table class=\"fileContainer\" style=\"border:none;\"><tbody><tr>"+
		 "<th><div class=\"fileIcon <vwb:FileExtend  fileName='${curVersion.title}'/>\"></div></th>"+
		 "<td><p class=\"fileNote\"></p><div class=\"largeButtonHolder\">"+
                 "<p class=\"fileName\">${curVersion.title}</p>"+
                 "<p>浏览器不支持该文件类型的在线显示</p>"+
		 "<a href=\"${downloadURL}\" class=\"largeButton extra\">"+
                 "下载<span class=\"ui-text-note\">(${sizeShort})</span>"+

		...
		if(checkHtml5()||isIE9()){
			<c:choose>
				 <c:when test="${pdfstatus == 'original_pdf'}">
				 	var url = "<vwb:Link page='${resource.rid}' context='f' format='url'/>?func=onlineViewer&offPresentationMode=true&version=${curVersion.version}";
			     </c:when>
			    <c:otherwise>
			...
			$("#fileInfo").append("<table class=\"fileContainer\" style=\"border:none;\"><tbody><tr>" +
				"<th><div class=\"fileIcon <vwb:FileExtend  fileName='${curVersion.title}'/>\"></div></th>" +
				"<td><p class=\"fileNote\"></p><div class=\"largeButtonHolder\"><p class=\"fileName\">${curVersion.title}</p><p>团队文档库在线预览服务所支持的浏览器包括Chrome、IE9+、Firefox和Safari。请确认您使用了合适的浏览器并且没有使用IE兼容模式。</p>" +

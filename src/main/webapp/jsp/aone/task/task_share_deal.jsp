...
<body>
	<div id="pagecontent">
		...
		<b>${task.title }</b><br>
		<form id="editform" name="editform" method="post" action="<vwb:Link context='task'  format='url'/>/share?func=dealSubmit">
			<input type="hidden" name="taskId" value="${task.taskId }"/>
			<table id="undo">
			<tr>
				<td colspan="2" ><b>未接受</b></td>
			</tr>
			<c:forEach items="${items }" var="item">
				<c:if test="${item.status=='undo' }">
					<tr <c:if test="${item.canEdit }">id="${item.rid }"</c:if>>
						<td>${item.content }</td>
						<c:choose>
						...
						<c:otherwise>
							<td>已于[${item.editTime }]被[${item.userNameStr }] 处理！</td>
						</c:otherwise>
						</c:choose>
					</tr>
				</c:if>

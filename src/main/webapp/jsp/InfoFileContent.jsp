...
$(document).ready(function(){
	...
	$(".recoverPageVersion").live("click",function(){
		...
		if (aa) {
			window.location.href="<vwb:Link context='infoFile' format='url' page='${rid}'/>"+"&func=recoverVersion&tid="+tid+"&version="+version;
		}
	});
...
<div class="content-major">
	...
	<vwb:CheckVersion mode="notfirst" rid="${rid}">
		...
		<vwb:SetPagination start="${startitem}" total="${itemcount}" pagesize="${pagesize}" maxlinks="9"  rid="${rid}" urlPatterns="infoFile" fmtkey="info.pagination" href='${baseURL})' />
			...
			<tbody>
				<c:forEach items="${versionList}" var="currentPage">
						<tr>
							...
							<td title="${currentPage.editor}">${users[currentPage.editor]}</td>
							<td>
							...
							<td>
								<a href="<vwb:Link context='download' format='url' page='${rid}'/>?version=${currentPage.version}">下载</a>
							</td>
						</tr>
				</c:forEach>
			</tbody>

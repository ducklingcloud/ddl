...
<div class="lionContent" style="width:1158px;">
	<div class="content-menu readyHighLight0" >
		<ul class="myNavList">
		    <li class="filterDrop">
              <a id="showAllFiles" class="" href="${contextPath}/pan/list">所有文件</a>
              <div class="clear"></div>
            </li>
		</ul>
	...
	<div class="content-right-board">
		...
		<div id="fileItemDisplay" >
			<ul id="resourceList" class="readyHighLight1 asTight"> 
				<c:forEach items="${srs }" var="re">
		   		<c:set value="${res[re.id] }" var="share"></c:set>
				<li class="files-item element-data">
					...
					<div class='oper' style="width:35px; width:50px\0;">
						...
						<c:if test="${!empty re.password }">
							<div class="iconLynxTag icon-codeLock"></div>
						</c:if>
						<input type='hidden' class='rid' value='${re.id }'/>
					</div> 
				    <span class="file-commands" style="display: none;">
				           	<a class="cancle-share cancelShare" title="取消分享" style="float:left"></a>
					</span>
						<h2>
							<a class='fileName' title='${share.title }' href="${shareUrl[re.id] }" target="_blank">
								<span class=' ${share.itemType} headImg ${share.fileType }'><input type='hidden' class='itemType' value='${share.itemType}'></span>
								<span class="fileNameSpan">${share.title}</span>
								<c:if test="${empty share.path }"><span style="color: red">(已失效)</span></c:if>
			            	</a>
						</h2>
						...
						<div class='time-col col'>
							<c:choose>
								<c:when test="${empty re.password }">
									<span style="display:none;" id="codeText-${re.id }">${shareUrl[re.id] }</span>
									<span class="fetchCode"> &nbsp; (无)</span>
								</c:when>
								<c:otherwise>
									<span style="display:none;" id="codeText-${re.id }">${shareUrl[re.id] } 提取码：${re.password}</span>
									<span class="fetchCode">${re.password}</span>
								</c:otherwise>
							</c:choose>
							<span id="copyCode-${re.id }" class="copyFetchCode" data-clipboard-target="codeText-${re.id }">复制</span>
						</div>
				       <div class='creator-col col'>
							<span>${user}</span>
						</div>
				       <div class='time-col col'><span><fmt:formatDate value="${re.shareTime }" pattern="yyyy-MM-dd HH:mm"/></span></div>
					   <div class='version-col col'>
							<span>
								<c:choose>

...
<div id="content-major" style="width:100%;margin-top:35px;padding-bottom:20px;">
	<div id="content-title">
		...
		<h1 class="fileName" style="font-size:22px; margin-bottom:0px;">
			<div class="title-right" >
				<a class="btn btn-success" href="${downloadURL}" style="color:#fff;">
					<i class="icon-download-alt icon-white"></i> 下载 <span style="font-size:12px;">(${sizeShort})</span>
				</a> 
				
			</div>
			<div class="title-left" style="margin-right:90px">
				<div class="${resource.itemType} headImg40 ${resource.fileType}"></div>
				<span id="pageTitle" rid="${resource.rid}"><c:out value="${resource.title}"/></span>
				<div id="version">
					分享者：<span>${shareUserName}</span> &nbsp;|&nbsp;
					分享时间： <fmt:formatDate value="${resource.createTime}" type="both" dateStyle="medium" />
					 &nbsp;|&nbsp; ${sizeShort}
				</div>
			</div>
		</h1>
	...
	<c:choose>
		...
		<c:when test="${fileExtend eq 'TEXT'}">
			<div id="codeMode">
				<pre id="directShowFile"><vwb:ShowPanShareFile panShareId="${shareResource.panShareId}" /></pre>
			</div>
		</c:when>
		<c:otherwise>
			<div id="photoInfo">
		...
		if(checkHtml5()||isIE9()){
			<c:choose>
				 <c:when test="${pdfstatus == 'original_pdf'}">
				 	var url = window.location.href+"?func=pdf";

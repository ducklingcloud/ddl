...
public class ShowPanShareFileTag extends VWBBaseTag {
	...
	public int doVWBStart() throws Exception {
		IPanService ps = DDLFacade.getBean(IPanService.class);
			try{
				...
				PanShareInfo meta = ps.getShareMeta(panShareId);
				ps.getShareContent(panShareId, null, ous);
				DirectShowFileSaver fs = new DirectShowFileSaver(meta.getMeta().getBytes());
				InputStream in = new ByteArrayInputStream(ous.toByteArray());
				fs.save(meta.getName(), in);
